...
function setLoading() {
    $('#contact-form-submit').removeClass('success').addClass('loading');
    $('#contact-form input').removeClass('error');
    setTimeout(function() {
        $('.loading').css('animation', 'button-loading 1s infinite linear');
    }, 200);
}

function setSuccess() {
    $('#contact-form-submit').removeClass('loading').addClass('success').css('animation', 'none');
    $('#contact-form input').removeClass('error');
    $('svg.checkmark').css({
        'stroke-dashoffset' : 0,
...
function setError() {
    $('#contact-form-submit').removeClass('loading').addClass('error').css('animation', 'none');
    $('svg.cross').css({
        'stroke-dashoffset' : 0,
        'transition' : 'stroke-dashoffset 600ms linear',
        'opacity' : 1
...
function removeErrors() {
    ...
    $('#contact-form-submit').removeClass('loading success error').css('animation', 'none');
    $('svg.cross, svg.checkmark').css({
...
function getContactData() {
    return {
        ...
        'comment' : $('#contact-form-comment').val()
    };
}
...
function contactError(jqXhr) {
    var result = JSON.parse(jqXhr.responseText);

    $.each(result, function(field, isGucci) {
        if (!isGucci) {
            $('#contact-form-' + field).addClass('error');
        }
    });
...
$(function() {
    $('#contact-form').on('submit', function(e) {
        ...
        setLoading();
        setTimeout(function() {
            $.post('contact-form.php', getContactData())
             .then(contactSuccess, contactError);
        }, 1500);
    });

    $('#contact-form input, #contact-form textarea').on('keydown keyup change', removeErrors);
});
...
function initMap() {
    var myLatLng = {lat: 51.446566, lng: 5.457580};

    var map = new google.maps.Map(document.getElementById('lommap'), {
        center: myLatLng,
        zoom: 16,
        styles: styles,
        scrollwheel: false,
